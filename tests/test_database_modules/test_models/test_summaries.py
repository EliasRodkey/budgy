...
def test_summaries_table_columns(clean_summaries_database):
    """SummariesTable DataFrame contains id, date, month, year, and primary category columns."""
    summaries_db, _ = clean_summaries_database
    df = summaries_db.to_dataframe()

    required_columns = ["id", "date", "month", "year"]
...
def test_summary_month_year_uq_constraint(clean_summaries_database):
    ...
    try:
        ...
        assert isinstance(e, DatabaseIntegrityError), f"Wrong error or no error thrown! {e}"
